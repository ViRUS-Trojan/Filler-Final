...
void	render_scoreboard(t_envars *e, t_gslst *gs)
{
	...
	mlx_string_put(e->mlx, e->win, e->b_lpad, 10, 0x18FF0A, e->g->p1);
	mlx_string_put(e->mlx, e->win, e->b_lpad + p1_end, 10, 0x18FF0A, p1);
	mlx_string_put(e->mlx, e->win, e->b_lpad, 30, 0xFE1537, e->g->p2);
	mlx_string_put(e->mlx, e->win, e->b_lpad + p2_end, 30, 0xFE1537, p2);
	free(p1);
	free(p2);
}
...
void	render_piece_pos(t_envars *e, t_gslst *gs)
{
	...
	if (gs->prev)
	{
		...
		if (gs->prev->flags & P1_PLACED)
			mlx_string_put(e->mlx, e->win, x_start, 10, 0x18FF0A, p_str);
		else
			mlx_string_put(e->mlx, e->win, x_start, 10, 0xFE1537, p_str);
		free(x);
		free(y);
		free(p_str);
	}
